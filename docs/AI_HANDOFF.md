...
- Clean Phase 3 production integration is prepared but not released on `phase3-main-integration`, from reference integration commit `651b055662c255bbb9376995fec8b01622e6a82d`. Its sole Phase 3 semantic source is `data/phase3/learner-content-499-authored.json`: 499 externally semantic-authored exact-sense aids, including 14 v2 sense corrections. Phase 2C remains 99 reviewed aids; the generated runtime contains 598 records. `travers|NOM` and `cesse|NOM` remain search-only. The Phase 3 UI intentionally does not show `已审核`; full research history remains on `phase3-scale-prep`.
