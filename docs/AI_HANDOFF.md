...
- Phase 2 pilot is local-only and not connected to runtime or deployment: `data/learner-content-pilot.json` contains exactly 20 AI-draft/blocked, sense-bound learner-content records.  It is generated and checked by `scripts/build_learner_content_pilot.py` and `scripts/validate_learner_content_pilot.py`; the latter verifies SQLite senses, sourced relation edges, target forms, status/provenance, and deterministic regeneration.  Pilot identity needs `lexeme_id + entry_id + sense_id`, because some source sense identifiers are not unique within a lexeme.
