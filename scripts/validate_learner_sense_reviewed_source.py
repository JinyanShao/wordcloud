...
def main():
 ...
 for x in rows:
  if x['content_status']=='reviewed':
   if not x['entry_id'] or not x['sense_id'] or x.get('review_provenance',{}).get('review_status')!='external_semantic_reviewed': raise SystemExit('invalid reviewed identity/provenance')
